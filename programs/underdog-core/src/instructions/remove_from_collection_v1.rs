...
use mpl_bubblegum::{
  cpi::{accounts::CollectionVerification, unverify_collection},
  program::Bubblegum,
};
...
pub struct RemoveFromCollectionV1Args {
...
#[derive(Accounts)]
#[instruction(args: RemoveFromCollectionV1Args)]
pub struct RemoveFromCollectionV1<'info> {
  #[account(mut)]
...
impl<'info> RemoveFromCollectionV1<'info> {
  fn unverify_collection_ctx(
    &self,
    is_delegated: bool,
  ) -> CpiContext<'_, '_, '_, 'info, CollectionVerification<'info>> {
    let cpi_accounts = CollectionVerification {
      tree_authority: self.tree_authority.to_account_info(),
      leaf_owner: self.recipient.to_account_info(),
      leaf_delegate: if is_delegated == true {
...
pub fn handler<'info>(
  ...
  unverify_collection(
    ctx
      .accounts
      .unverify_collection_ctx(args.is_delegated)
