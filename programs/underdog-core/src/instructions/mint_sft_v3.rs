...
#[derive(Accounts)]
#[instruction(args: MintSftV3Args)]
pub struct MintSftV3<'info> {
  #[account(mut)]
...
pub fn handler(ctx: Context<MintSftV3>, args: MintSftV3Args) -> Result<()> {
  ...
  let project_metadata: &Box<Account<'_, TokenMetadata>> = &ctx.accounts.project_metadata;

  let metadata = MetadataArgs {
    name: project_metadata.data.name.trim_end_matches('\0').to_owned(),
    symbol: project_metadata
      .data
      .symbol
      .trim_end_matches('\0')
      .to_owned(),
    uri: project_metadata.data.uri.trim_end_matches('\0').to_owned(),
