...
export function amountAsNum(amount: BN | bigint, mintOrDecimals: Mint | number): number {
  const decimals: number =
    typeof mintOrDecimals === "number" ? mintOrDecimals : (mintOrDecimals as Mint).decimals;
...
export function toBN(numberOrBn: BN | number, mintOrDecimals: Mint | number): BN {
  const decimals: number =
    typeof mintOrDecimals === "number" ? mintOrDecimals : (mintOrDecimals as Mint).decimals;
  ...
  if (BN.isBN(numberOrBn)) {
    ...
    return new BN(Number(Math.floor(Number(numberOrBn))))
      ...
      .add(new BN(Math.round((Number(numberOrBn) % 1) * Math.pow(10, decimals))));
  }
}
...
export function humanReadable(bn: BN, decimalsOrMint: Mint | number): string {
  return numberWithCommas(
    roundToDecimals(
      ...
      typeof decimalsOrMint == "number" ? decimalsOrMint : decimalsOrMint.decimals
    )
  );
}
